...
<head>
    ...
    <link rel="canonical" href="https://allenhouseghaziabad.com/awards" />
    <title>Awards & Recognitions | AllenHouse Ghaziabad</title>
    ...
    <div class="main relative mb-[40px] sm:mb-[120px]">
        ...
        <div class="flex m-5" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li class="text-xs font-medium text-blue-main uppercase tracking-tighter">Awards</li>
            </ol>
        </div>
            ...
            <div class="tabs">
                <div class="flex flex-col sm:flex-row items-center gap-4 sm:justify-between border-b pb-4 border-gray-200">
                    <ul class="flex gap-2 bg-white p-1 rounded-xl shadow-sm border border-gray-100">
                        <li>
                            <a href="javascript:void(0)" class="tab-link active-tab block py-2 px-6 font-bold rounded-lg text-xs sm:text-sm uppercase tracking-widest">Achievements Gallery</a>
                        </li>
                    </ul>
                    <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-[50%]">
                        ...
                        <input type="text" id="searchInput" class="bg-white w-full border border-gray-200 px-5 py-3 outline-none rounded-xl text-sm shadow-sm focus:ring-2 focus:ring-blue-main/20" placeholder="Search by event or year..." />
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadAwards() {
            try {
                ...
                awardData = all.filter(isGalleryAwardItem);
                populateGalleryYearSelect(document.getElementById('yearFilter'), awardData);
                applyAwardFilters();
            } catch (err) {
                console.error("Ghaziabad Awards API Error:", err);
            }
        }

        function applyAwardFilters() {
            let list = filterGalleryItemsByYear(awardData, currentYear);
            if (searchTerm.trim()) {
        ...
        function renderGallery(data) {
            ...
            if (data.length === 0) {
                container.innerHTML = '<div class="col-span-full py-20 text-center text-gray-400">No awards or recognitions currently available for Ghaziabad.</div>';
                return;
            }
                ...
                <div class="achievement-card flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-full">
                    <div class="relative overflow-hidden bg-gray-50">
                        <a href="achievement-gallery?id=${item.id}">
                            ${isPdf ? `
                                <div class="flex flex-col items-center justify-center p-12 h-[250px] border-b">
                                    ...
                                    <span class="text-[10px] font-bold text-gray-400 mt-2 uppercase tracking-tighter">View Document</span>
                                </div>
                            ` : `
                                <img src="${imgUrl || 'https://via.placeholder.com/600x400'}" alt="${item.achievementtitle}" class="w-full h-[250px] object-cover transition-transform duration-700 hover:scale-110" />
                            `}
                        </a>
                        <div class="absolute top-4 left-4">
                            <span class="bg-blue-main text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter shadow-sm">Campus Pride</span>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        <div class="flex gap-4 mb-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-[10px] text-gray-400 uppercase tracking-widest mt-1 font-semibold">AllenHouse Ghaziabad</p>
                            </div>
                        </div>

                        <div class="mt-auto pt-5 border-t border-gray-50">
                            <a href="achievement-gallery?id=${item.id}" class="w-full py-3 bg-blue-main text-white text-[11px] font-black rounded-xl hover:bg-slate-800 transition-all uppercase flex items-center justify-center gap-2">
                                EXPLORE GALLERY
        ...
        window.addEventListener('DOMContentLoaded', loadAwards);
    </script>
</body>
</html>