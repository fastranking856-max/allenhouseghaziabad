...
<head>
    ...
    <link rel="canonical" href="https://allenhouseghaziabad.com/xii-toppers" />
    <title>Class XII Board Toppers | AllenHouse Ghaziabad</title>
...
<body class="bg-slate-50">
    ...
    <div class="main relative mb-[40px] sm:mb-[120px]">
        ...
        <div class="flex m-5" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <svg class="w-3 h-3 text-blue-main mx-1" fill="none" viewBox="0 0 6 10"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/></svg>
                <li class="text-xs font-medium text-blue-main uppercase tracking-tighter">XII Toppers</li>
            </ol>
        </div>
            ...
            <div class="tabs">
                <div class="flex flex-col sm:flex-row items-center gap-4 sm:justify-between border-b pb-4">
                    <ul class="flex gap-2 bg-white p-1 rounded-xl shadow-sm border border-gray-100">
                        <li>
                            <a href="javascript:void(0)" class="tab-link active-tab block py-2 px-6 font-bold rounded-lg text-xs sm:text-sm uppercase tracking-widest">Merit List</a>
                        </li>
                    </ul>
                    <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-[50%]">
                        ...
                        <input type="text" id="xiiSearch" class="bg-white w-full border border-gray-200 px-5 py-3 outline-none rounded-xl text-sm shadow-sm focus:ring-2 focus:ring-blue-main/20" placeholder="Search by name or year..." />
                    </div>
                </div>
    ...
    <?php include "includes/foot.php" ?>
     ...
     <script>
        ...
        async function loadXIIToppers() {
            try {
                const all = await fetchAllCmsGalleriesByBranch();
                xiiTopperData = all.filter(function(item) {
                    return gallerySubTypeName(item) === 'xii_toppers';
                });
                populateGalleryYearSelect(document.getElementById('yearFilter'), xiiTopperData);
                applyTopperFilters();
            } catch (err) {
                console.error("Ghaziabad XII API Error:", err);
            }
        }

        function applyTopperFilters() {
            let list = filterGalleryItemsByYear(xiiTopperData, currentYear);
            if (searchTerm.trim()) {
        ...
        function renderToppers(data) {
            ...
            if (data.length === 0) {
                container.innerHTML = '<div class="col-span-full py-20 text-center text-gray-400 font-medium tracking-wide">No Class XII records found for Ghaziabad campus.</div>';
                return;
            }

            data.forEach(item => {
                const cover = item.medias?.find(m => m.pivot?.is_cover === "1") || item.medias?.[0];
                const imgUrl = cover ? cover.urls : 'https://via.placeholder.com/600x400?text=XII+Topper';
                ...
                <div class="topper-card flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-full">
                    <div class="relative aspect-[4/3] overflow-hidden bg-gray-100">
                        <img src="${imgUrl}" alt="${item.achievementtitle}" class="w-full h-full object-cover transition-transform duration-700 hover:scale-110" />
                        <div class="absolute top-4 left-4">
                            <span class="bg-blue-main text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">Merit Holder</span>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        <div class="flex gap-4 mb-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-[10px] text-gray-400 uppercase tracking-widest mt-1 font-semibold">Board Topper | Class XII</p>
                            </div>
                        </div>
        ...
        document.getElementById('xiiSearch').addEventListener('input', function(e) {
        ...
        window.addEventListener('DOMContentLoaded', loadXIIToppers);
    </script>
</body>
</html>